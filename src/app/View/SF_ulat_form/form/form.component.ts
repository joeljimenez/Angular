...
export class FormComponent implements OnInit {
  ...
  ngOnInit() { 
    ...
    this.route.params.subscribe(params => {
      if (params['data'] != null) {
        try {
          this.solicitud.hash = params['data'].replace('°', '/');
        }catch (error) {
  ...
  muestra_confirmacion(){
    let mensaje ="<p>El evaluado(a) <strong> " + this.encabezado_pagina.nombre + " </strong>  ha contestado y confirmado  la evaluación ya no podrá ser modificada .</p>" +
    "<p> Saludos cordiales…</p>";

    this.solicitud.metodo = 1,
    this.solicitud.consulta = 5,
    this.solicitud.idevaluacion =  this.encabezado_pagina.idevaluacion,
    this.solicitud.email =  this.encabezado_pagina.email,
    this.solicitud.email2 = this.encabezado_pagina.email2,
    this.solicitud.link = mensaje,
    this.solicitud.nombre = 'Confirmación de evaluación'


    this.titulo_modal = 'CONFIRMACIÓN DE LA REVISIÓN';

    this.mensaje = 'Al Presionar el botón OK, confirma que sido evaluado objetivamente con el propósito de detectar las debilidades y fortalezas para la mejora del personal.<br><br> Automáticamente esta evaluación será cerrada y no podrá ser modificada por ninguna de las dos partes. Para asegurar su evaluación se descargará un documento con la misma.';
  ...
  envio_correo(a) {
    this.loading = true;
    if (this.valida_respuestascompletas()) {
        let link = window.location.origin + "/home/form?hash=" +  this.datos_envio.hash;
