...
(function (global, factory) {
    ...
    //                    _radio = lineatexto;
    //                    lineatexto = 1;
    //                    coordenadastexto.push({ x: x, y: y, data: _data[z] });
    //                }
    //            }
    ...
    //    this.circle(_left, _top, _radio / 4, 'FD');
    //    tiempo = 0;
    //    this.setFontSize(12);
    //    for (var t in coordenadastexto) {
    //        this.text(coordenadastexto[t].data.toFixed(2) + '%', coordenadastexto[t].x, coordenadastexto[t].y)
    //    }
    ...
    jsPDF.API.MasterPage = function (_fn) {
        if (typeof _fn == 'function') {
            
            var rt = {
                page: 0,
                page_count: this.internal.getNumberOfPages(),
                settings: settings
            }
            for (var page = 1; page <= this.internal.getNumberOfPages() ; page++) {
                this.setPage(page);
                rt.page = page;
                _fn(rt);
            }
            ...
            console.error('The master page receives a function to decorate the pages of the pdf and returns the page number that is being modified.');
        }
